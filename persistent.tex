...
We can now see that if we have a persistence module $M$ over some ring $R$ then a graded module structure over $R[t]$ on $M$ is given by
...
\begin{algorithmic}[1]
  ...
  % \For{$i < n$}
  % \If{column $i$ is not zero}
  \While{there are untreated non-zero rows or columns}
...
\end{tabular}
\end{center}
  % \[v_{2}+v_{1},t(v_{1})+t^{2}(v_{0}),t(v_{2})+t^{2}(v_{0}),v_{3}+t^{2}v_{0}, t^{2}(v_{12})+t(v_{10})+t(v_{02}).\]
...
%       \to
% \bordermatrix{
%     & v_{0} & v_{1} & v_{2} & v_{3} & v_{12}  & v_{02} + tv_{12}& v_{01} & v_{03} & v_{012}\cr
%     v_{0}  &.&.&.&.&.&t^{2}&t^{2}&t^{2}&.\cr
%     v_{1}  &.&.&.&.&1&t&t&.&.\cr
%     v_{2}  &.&.&.&.&1&.&.&.&.\cr
%     v_{3}  &.&.&.&.&.&.&.&1&.\cr
