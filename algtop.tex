...
An \textbf{abstract simplicial complex} $K$ is a finite collection of ordered sets such that $\alpha \in K$ and $\beta \subseteq \alpha$ implies that $\beta \in K$.
\end{definition}
...
\begin{definition}
Given a geometric simplicial complex $K$, we can construct an abstract simplicial complex $A$ by translating each simplex $\sigma_{i} \in K$ to an ordered set $[v_{0},\dots,v_{n}]$ where each $v_{i}$ is an algebraic object simply denoting the presence of a vertex. We call $K$ a \textbf{geometric realization} of $A$.
\end{definition}
...
\begin{definition}[\cite{Zomorodian2005}]\label{chainmodulesimp}
  ...
  where $r_{i} \in R$ and $\sigma_{i}$ is a $k$-dimensional simplex in $K$. Furthermore, there is an equivalence of elements in the chain module such that $\sigma = -\tau$ if $\tau$ is given by an even permutation of the vertices of $\sigma$.
\end{definition}
...
\end{tikzcd}
\end{center}
where we define the differential $\partial_{k}$ is defined as below and where $[v_{0},\dots,\hat v_{i}, \dots, v_{k}]$ denotes the face of the simplex given by dropping the vertex $v_{i}$.
\begin{align*}
