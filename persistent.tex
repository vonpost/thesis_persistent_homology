...
\[
  ...
  % \]
  where the maps are the obvious inclusions and projections from and onto components and the isomorphism through the decomposition of $M$. Then for at least one pair $i,j$ the homomorphism $f_{ij}$ cannot be the zero map since $\sum_{i,j} \iota_{C_{i}} g_{ij} f_{ij}: C \to C$ is an automorphism of $C$. Assume that $i=1,j=1$, if this is not the case we can just permute the summands.  Then $f_{11}$ (by symmetry all arguments below hold for $g_{ij}$ as well) is an isomorphism $C_{1} \cong D_{1}$ since the graded isomorphism $\phi$ takes a homogeneous element $t^{a} \to c \cdot t^{a}$ for some $c \in \mathbb{F}$.

  Furthermore, we claim that the map $f_{2_{+}}$ defined below is also an isomorphism
  \[ f_{2_{+}}: \oplus_{i=2}C_{i} \to \oplus_{j=2} D_{j} \]
  \[f_{2_{+}}(c_{2},c_{3},\dots) = \sum_{i=2,j=2} \iota'_{D_{j}}f_{ij}(c_{i}) \] where $\iota'_{D_{j}}$ denotes the inclusion $D_{j} \hookrightarrow \bigoplus_{j=2} D_{j}$.

  For injectivity, note that if $\hat c = (c_{2},c_{3},\dots) \in \ker f_{2+}$ then $\phi$ has to map $\sum_{i=2} \iota_{C_{i}} \hat c=(0,c_{2},c_{3},\dots)$ to $(d_{1},0,0,\dots)$ for some $d_{1} \in D_{1}$ since the final maps are just the projections onto components and inclusion into $\oplus_{j=2}D_{j}$. By post-composing with $\pi_{C_{1}} \phi^{-1}$ we get

  \[(d_{1},0,0,\dots) = \iota_{D_{1}}(d_{1})= \phi \sum_{i=2}\iota_{C_{i}}(\hat c) \iff
  \]
  \[
    \pi_{C_{1}} \phi^{-1} \iota_{D_{1}}(d_{1})=  \pi_{C_{1}} \phi^{-1} \phi \sum_{i=2}\iota_{C_{i}}(\hat c) \iff
      g_{11}(d_{1}) = 0
    \]
    ...
    \begin{align*}
       f_{2_{+}}(c_{2},c_{3}, \dots) = & \sum_{i=2,j=2} \iota'_{D_{j}}f_{ij}(c_{2},c_{3},\dots) &= \\
      \sum_{i=2,j=2} \iota'_{D_{j}}\pi_{D_{j}} \phi \iota_{C_{i}} (c_{2},c_{3},\dots) = &  \sum_{j=2} \iota'_{D_{j}}\pi_{D_{j}} \phi \sum_{i=2} \iota_{C_{i}} (c_{2},c_{3},\dots)  &=\\
                                       \sum_{j=2} \iota'_{D_{j}}\pi_{D_{j}} (d_{1},d_{2},\dots)   = & \hat d
    \end{align*}

  So we get that $f_{2_{+}}$ is an isomorphism and by symmetry $g_{2+}$ is also an isomorphism. Hence, $g_{2+} f_{2_{+}}$ is an automorphism of $\bigoplus_{i=2} C_{i}$ and we can repeat the same argument to find new indices $i,j$ such that $f_{ij}$ and $g_{ij}$ are isomorphisms. Since the decompositions $C,D$ are direct sums there are a finite number of non-trivial modules thus this process will eventually exhaust all of them. The rest of the summands in the decompositions not covered by these isomorphisms must then consist of trivial modules which proves the uniqueness of the decomposition up to permutation and trivial modules.
